...
- [x] Field-test reconnect: unplug/replug the panel and confirm recovery
      *(2026-08-18: first test did NOT recover — an unplug surfaces as a soft
      decline on macOS. Fixed via HID presence check + decline time cap;
      re-test passed: detected on the 2nd declined frame, reconnected 40 s
      after the replug — see docs/project-status.md)*
